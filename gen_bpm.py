...
def generar_datos():
    ...
    while True:
        ...
        if evento_activado and now >= fin_evento:
            ...
            tiempo_sin_riesgo = now + timedelta(seconds=30)  # 30 segundos sin riesgo

        # Si hay tiempo sin riesgo
        if tiempo_sin_riesgo and now >= tiempo_sin_riesgo:
            print("✅ Sin riesgo")
            evento_activado = None  # Estado normal
        ...
        try:
            ...
            cursor.execute(query, (id_user, bpm, spo2, evento_activado if evento_activado else "✅ Sin riesgo", now))
            conn.commit()
            cursor.close()
            conn.close()
            print(f"{evento_activado if evento_activado else '✅ Sin riesgo'} - BPM: {bpm}, SpO2: {spo2}, Hora: {now}")
